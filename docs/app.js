...
$("clear").addEventListener("click",()=>{$("source").value="";$("url").value="";$("text").value="";$("results").hidden=true;$("verify").hidden=true;$("error").hidden=true;currentAnalysis=null;evidenceMatrices={};});
...
$("analyze").addEventListener("click",async()=>{
  ...
  try{
    if(url){const r=await fetch(`${API_BASE}/api/ingest-url`,{method:"POST",headers:{"Content-Type":"application/json","Accept":"application/json"},body:JSON.stringify({article_url:url})});const data=await r.json().catch(()=>null);if(!r.ok)throw new Error(data?.detail||`The source could not be analyzed (HTTP ${r.status}).`);currentAnalysis=data;evidenceMatrices={};render(data);$("verify").hidden=!(data.evidence_link_count>0);return;}
    const claims=await extractCandidateClaims(text);currentAnalysis={record_id:`ai_preview_${Date.now().toString(36)}`,source_name:source||null,article_url:null,claims,factual_claim_count:claims.length,methodology_note:"Local text mode: claims are extracted in your browser. Deep verification requires a persisted source URL."};render(currentAnalysis);$("verify").hidden=true;
